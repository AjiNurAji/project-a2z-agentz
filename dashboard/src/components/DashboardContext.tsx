...
// --- Generators ---
const PROJECTS = ["ZeroGravity Protocol","NeuralFi","BaseSwap V3","OmniLayer DAO","CryptoNest","DeFi Nexus","ChainLink Base","Arbitrage Bot X","Yield Optimizer Pro","FlashLoan Gate"];
const FARCASTER_MSGS = [
  "📡 Farcaster channel /defi scanned — 342 casts analyzed",
...
function randFrom<T>(arr: T[]): T { return arr[randInt(0, arr.length - 1)]; }
function genId() { return Math.random().toString(36).slice(2, 10); }
function genAddress() { return "0x" + Array.from({ length: 40 }, () => "0123456789abcdef"[randInt(0,15)]).join(""); }
function genTxHash() { return "0x" + Array.from({ length: 64 }, () => "0123456789abcdef"[randInt(0,15)]).join(""); }
...
export function DashboardProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (roll < 0.5) {
        ...
        addLog(newTx.status === "success" ? "SUCCESS" : "ERROR", 
          newTx.status === "success" 
            ? `✅ Tx confirmed: ${newTx.projectName} +$${newTx.amountUsd} | Hash: ${newTx.txHash.slice(0, 16)}...`
            : `❌ Tx failed: ${newTx.projectName} — RPC timeout, retry scheduled`
        );
      }
    }, 3000);
  ...
  const handleReject = useCallback((id: string) => {
    const item = approvalQueue.find(a => a.id === id);
    setApprovalQueue(prev => prev.filter(a => a.id !== id));
    if (item) addLog("WARN", `⛔ Rejected: ${item.projectName} — human override`);
  }, [approvalQueue, addLog]);
  ...
  const handleClearCache = useCallback((id: string) => {
