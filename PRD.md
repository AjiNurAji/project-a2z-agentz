...
> **Runtime note**: The system is architected for AMD vLLM / MI300X; **current live production and demo inference runs on Llama/Llama-3.1-8B-Instruct-AWQvia AMD vLLM**.
...
graph TD
 ...
 O[On-Chain Block Explorer]
    ...
    subgraph AMD Developer Cloud (Agent A - The Scout)
        AW[AMD AI Workbench<br/>Llama/Llama-3.1-8B-Instruct-AWQInference]
...
- **AI Model**: Llama/Llama-3.1-8B-Instruct-AWQ(via AMD vLLM) → **fine-tuned via AMD AI Workbench** on a Web3 sentiment dataset (output: "vLLM-served LLM" / "A2Z-tuned model").
- **Database**:
...
* **AMD-Native AI Stack**:
  * **Model**: vLLM-served LLM (Llama/Llama-3.1-8B-Instruct-AWQ(via AMD vLLM) via AMD AI Workbench on ~5,000–10,000 labeled examples from Farcaster and on-chain narrative).
...
- [ ] Fine-tune Llama/Llama-3.1-8B-Instruct-AWQon **MI300X** (vLLM target); live demo runs via **AMD vLLM** via AMD AI Workbench
